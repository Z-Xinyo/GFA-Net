...
skeleton_folder = '/dadaY/xinyu/dataset/pkummd/v1/PKU_Skeleton_Renew/' #骨骼数据文件夹的路径
label_folder = '/dadaY/xinyu/dataset/pkummd/v1/Label/Train_Label_PKU_final/' #标签文件夹的路径
save_path = '/dadaY/xinyu/dataset/ST_MGN_data/pkummd/skeleton_pku_v1/' #保存.skeleton文件的路径
...
def array_to_skl_lines(skeleton_frame):
    """将长度为75的一帧（25个关节×XYZ）转成多行字符串，每行一个关节的x y z"""
    skeleton_lines = ''
...
def transfer_and_save(file_id, file_view):
    """把某个视频ID与视角组合的数据切分成样本并保存为自定义.skeleton格式"""
...
for file_id in range(2, 365): # 遍历视频编号 2~364
