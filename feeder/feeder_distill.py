...
class Feeder(torch.utils.data.Dataset):
    ...
    def quantize_data(self, data_numpy):
        """
        量化数据，将数据从浮点数转换为int8格式，以减少数据量
        """
        if data_numpy is None:
            raise ValueError("Input data is None!")

        # 假设数据在范围 [min_val, max_val] 之间，归一化到 0-1 之间
        min_val = np.min(data_numpy)
        max_val = np.max(data_numpy)
        ...
        # 将数据归一化到 [0, 1]
        data_normalized = (data_numpy - min_val) / scale

        # 将归一化的数据量化到 int8 类型 [-128, 127]
        data_quantized = np.round(data_normalized * 255).astype(np.int8)
    ...
    def __getitem__(self, index):
        ...
        #data_numpy_v1 = self.quantize_data(data_numpy_v1)
        #data_numpy_v2 = self.quantize_data(data_numpy_v2)
