...
def train(train_loader, model, criterion1, optimizer, epoch, args):
    ...
    for i, (xq, xk) in enumerate(train_loader):
        ...
        # compute output
        output1, output2, output3, output4, output5, output6, target1, \
            target2, target3, target4, target5, target6 = model(xq, xk)

        batch_size = output1.size(0)

        # cross view loss
        loss1 = criterion1(output1, target1) + criterion1(output2, target2) + criterion1(output3, target3) +\
                criterion1(output4, target4)
        # cross domain loss
        ...
        acc1, _ = accuracy(output4, target4, topk=(1, 5))
