...
CUDA_VISIBLE_DEVICES=${cuda_device} python action_classification.py \
  --lr 0.1 \
  --batch-size 128 \
  --pretrained  ./checkpoints_distill/${test_name}/checkpoint_0450.pth.tar \
  --finetune-dataset ${dataset} --protocol ${protocol} \
  --finetune_skeleton_representation ${representation} | tee -a ./checkpoints_distill/${test_name}/classification.log